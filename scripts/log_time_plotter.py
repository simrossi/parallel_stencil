...
def plot_data(all_iterations_list, all_times_list, file_names):
    ...
    for i, (all_iterations, all_times, file_name) in enumerate(zip(all_iterations_list, all_times_list, file_names)):
        ...
        for iterations, times in zip(all_iterations, all_times):
            ax1.plot(iterations, times, color=color, marker='o', label=f"{file_name} (Cumulative)")
    ...
    for i, (all_iterations, all_times, file_name) in enumerate(zip(all_iterations_list, all_times_list, file_names)):
        color = colors[i % len(colors)]
        for iterations, times in zip(all_iterations, all_times):
            individual_times = [times[0]] + [times[i] - times[i - 1] for i in range(1, len(times))]
            ax2.plot(iterations, individual_times, color=color, marker='o', label=f"{file_name} (Individual)")
