...
float compute_stencil(const Matrix matrix, const uint32_t offset) {
  if (matrix.dimensions != stencil.dimensions) {
    fprintf(stderr, "Error: The dimensions of the provided matrix and stencil "
                    "are not the same.\n");
    exit(1);
  }
  ...
  for (uint32_t i = 0; i < matrix.dimensions; i++) {
    ...
    starts[i] =
        index -
        space_before; //(index > space_before ? index - space_before : 0);
    ends[i] = index + space_after; //(index + space_after < matrix.sizes[i] ?
                                   //index + space_after : matrix.sizes[i] - 1);
  ...
  // this part of code is fast enough even withou paralellization, adding an omp
  // pragma just adds useless overhead which makes the execution slower #pragma
  // omp parallel for num_threads(4) schedule(dynamic) private(curr,
  // stencil_matrix) shared(neighbors)
  for (uint32_t i = 0; i < region_size; i++) {
    ...
    float stencil_value =
        stencil.data[indices_to_offset(stencil_matrix, stencil_indices)];
    ...
    if (stencil_value != 0) {
      ...
      if (outside) {
        ...
        // Set the value to zero if the element affects the stencil while also
        // being outside of the matrix
        neighbors[actual_size] =
            matrix.data[indices_to_offset(matrix, closest_indices)] *
            stencil_value;
      } else {
        // Store the neighbors and scale them by the stencil value
        neighbors[actual_size] =
            matrix.data[indices_to_offset(matrix, (uint32_t *)curr)] *
            stencil_value;
      }
    ...
    for (uint32_t j = matrix.dimensions - 1; i != region_size - 1 && j > 0;
         j--) {
